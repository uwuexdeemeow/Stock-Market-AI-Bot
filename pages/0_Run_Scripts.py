...
Click into a tab, set parameters, click Run.  Output streams live
below.  Full logs saved to logs/dashboard_*.log.
"""
...
st.caption(
    ...
    "GitHub Actions cron — tweak as needed before clicking Run."
)
...
def _run_block(label: str, cmd: list[str], *,
    ...
    """Show resolved command + Run button."""
    if warning:
        st.warning(warning)
    if help_text:
...
# ─────────────────────────────────────────────────────────────────────
# QUICK ACTIONS — pinned at top
# ─────────────────────────────────────────────────────────────────────
st.markdown("### Quick actions")
qa1, qa2, qa3 = st.columns(3)

with qa1:
    if st.button("Daily run (Alpaca, full pipeline)", type="primary", use_container_width=True,
...
with qa2:
    if st.button("Refresh data + generate signal", use_container_width=True,
...
with qa3:
    if st.button("Show Alpaca account status", use_container_width=True,
...
# ── Tab 1: Daily pipeline ────────────────────────────────────────────
with tabs[0]:
    st.markdown("##### daily_run.py — full pipeline orchestrator")
    c1, c2 = st.columns(2)
    with c1:
    ...
    with c2:
        ...
        dr_report = st.checkbox("--report", value=False, key="dr_report",
                                 help="Side-by-side performance report at the end")
    ...
    _run_block(
        "Full daily pipeline", cmd, key="daily_run",
        warning=(None if dr_dry else "Submits paper orders if market is open."),
...
# ── Tab 2: Data Refresh ──────────────────────────────────────────────
with tabs[1]:
    st.markdown("##### refresh_etf_data.py")
    ...
    etf_symbols = st.text_input("--symbols (comma-separated, blank = default)",
    ...
    if etf_symbols.strip(): cmd.extend(["--symbols", etf_symbols.strip()])
    _run_block("Refresh ETF data", cmd, key="etf")

    st.divider()

    st.markdown("##### research.py — refresh factor panel")
    research_mode = st.radio(
        "Mode",
        ["incremental (fast)", "all (full rebuild)", "xs-only", "test"],
        index=0, horizontal=True, key="research_mode",
    )
    c1, c2 = st.columns(2)
    with c1:
        research_ticker = st.text_input("--ticker", value="", key="research_ticker")
    with c2:
        research_tickers = st.text_input("--tickers (space-separated)", value="", key="research_tickers")
    ...
    st.markdown("##### feature_quality_diagnostic.py")
    top_n = st.slider("--top N", min_value=10, max_value=100, value=48, step=2, key="fq_top")
...
# ── Tab 3: Signal ────────────────────────────────────────────────────
with tabs[2]:
    st.markdown("##### core_satellite_alpha.py — generate signal only")
    c1, c2 = st.columns(2)
    with c1:
        sig_ignore_stale = st.checkbox("--ignore-stale", value=False, key="sig_ignore_stale")
        sig_walkforward = st.checkbox("--walkforward (inline WF — SLOW)", value=False, key="sig_walkforward")
    with c2:
...
# ── Tab 4: Trading ───────────────────────────────────────────────────
with tabs[3]:
    st.markdown("##### alpaca_paper_trading.py --submit")
    c1, c2 = st.columns(2)
    with c1:
        ap_force = st.checkbox("--force (skip duplicate-day guard)", value=False, key="ap_force")
    ...
    st.markdown("##### alpaca_paper_trading.py --reconcile")
    st.caption("Check pending orders for fills/cancellations.")
    _run_block("Reconcile fills",
               [PY, "alpaca_paper_trading.py", "--reconcile"], key="reconcile")

    st.divider()

    st.markdown("##### alpaca_paper_trading.py --status")
    st.caption("Print positions/cash/equity. Read-only.")
    _run_block("Show account status",
               [PY, "alpaca_paper_trading.py", "--status"], key="status")

    st.divider()

    st.markdown("##### execution_guard.py")
    c1, c2 = st.columns(2)
    with c1:
        eg_once = st.checkbox("--once", value=True, key="eg_once")
    with c2:
        eg_dry = st.checkbox("--dry-run", value=False, key="eg_dry")
    ...
    _run_block("Execution guard", cmd, key="exec_guard",
               help_text="Repair stops, cancel stale orders, intraday P&L check.")


# ── Tab 5: Diagnostics ───────────────────────────────────────────────
with tabs[4]:
    st.markdown("##### broker_health.py")
    c1, c2, c3 = st.columns(3)
    with c1:
    ...
    st.markdown("##### alpaca_paper_gauntlet.py")
    c1, c2, c3 = st.columns(3)
    with c1:
    ...
    with c3:
        g_snapshot = st.checkbox("--snapshot", value=False, key="g_snapshot")
    ...
    st.markdown("##### walkforward_analyzer.py")
    wfa_csv = st.text_input("--csv", value="signals/core_satellite_nested_walkforward.csv", key="wfa_csv")
    c1, c2 = st.columns(2)
    with c1:
        wfa_qqq = st.text_input("--qqq", value="", key="wfa_qqq")
    with c2:
        wfa_spy = st.text_input("--spy", value="", key="wfa_spy")
    wfa_json = st.checkbox("--json", value=False, key="wfa_json")
...
# ── Tab 6: Heavy ops ─────────────────────────────────────────────────
with tabs[5]:
    st.markdown("##### Nested walkforward (2-6 HOURS)")
    st.warning(
    ...
    with c1:
        ...
        wf_full = st.checkbox("--full (exhaustive 768 configs)", value=True, key="wf_full")
        wf_fast = st.checkbox("--fast (~48 configs, ~15 min)", value=False, key="wf_fast")
    with c2:
        wf_stable = st.checkbox("--stable-grid (pinned ~24 configs)", value=False, key="wf_stable")
        wf_recent = st.checkbox("--recent-alpha-grid (~48 configs)", value=False, key="wf_recent")
        wf_output = st.text_input("--output-prefix", value="core_satellite_nested_walkforward", key="wf_output")

    c3, c4 = st.columns(2)
    with c3:
        wf_start_year = st.number_input("--start-year (0=auto)", min_value=0, max_value=2099,
                                         value=0, step=1, key="wf_start_year")
        wf_end_year = st.number_input("--end-year (0=auto)", min_value=0, max_value=2099,
    ...
    with c4:
        wf_max_folds = st.number_input("--max-folds (0=all)", min_value=0, max_value=20,
                                        value=0, step=1, key="wf_max_folds")
        wf_max_configs = st.number_input("--max-configs (0=all)", min_value=0, max_value=1000,
                                          value=0, step=10, key="wf_max_configs")
        wf_max_specs = st.number_input("--max-specs (0=default)", min_value=0, max_value=1000,
    ...
    st.markdown("##### Publish live config from walkforward CSV")
    c1, c2 = st.columns(2)
    with c1:
    ...
    with c2:
        pub_force = st.checkbox("--force", value=False, key="pub_force")
    ...
    if logs_dir.exists():
        ...
        if dashboard_logs:
            ...
            for log_path in dashboard_logs:
                ...
                with st.expander(f"{log_path.name}  ·  {mtime.strftime('%Y-%m-%d %H:%M:%S')}  ·  {size_kb:.1f} KB"):
                    try:
