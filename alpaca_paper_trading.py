...
# Plan CSV written after orders are computed (regardless of whether they
# actually get submitted).  Same path moomoo_paper_trading.py writes —
# downstream readers (paper_health, daily_paper_check, notifications,
# status, dashboard) work for both brokers off this one file.
...
def main():
    ...
    # paper_health, daily_paper_check, notifications, status, and the
    # dashboard all read signals/core_satellite_alpha_orders.csv to
    # display "what we plan to do today".  moomoo_paper_trading writes
    # the same file for its broker; this mirror keeps the file present
    # for Alpaca-only setups too (previously the file was missing and
    # the dashboard showed it as ⚫ missing).  Always write — even when
