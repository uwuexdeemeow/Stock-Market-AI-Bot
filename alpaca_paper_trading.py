...
def _execution_quality_summary(rows: list[dict]) -> dict:
    ...
    adverse_5 = [r["adverse_5m_bps"] for r in rows if r.get("adverse_5m_bps") is not None]
    adverse_15 = [r["adverse_15m_bps"] for r in rows if r.get("adverse_15m_bps") is not None]
    adverse_30 = [r["adverse_30m_bps"] for r in rows if r.get("adverse_30m_bps") is not None]
    adverse_60 = [r["adverse_60m_bps"] for r in rows if r.get("adverse_60m_bps") is not None]
    return {
        ...
        "adverse_5m_count": int(sum(value > 0 for value in adverse_5)),
        "adverse_5m_observations": len(adverse_5),
        "adverse_15m_count": int(sum(value > 0 for value in adverse_15)),
        "adverse_15m_observations": len(adverse_15),
        "adverse_30m_count": int(sum(value > 0 for value in adverse_30)),
        "adverse_30m_observations": len(adverse_30),
...
def _execution_quality_segments(rows: list[dict]) -> dict:
    ...
    return {
        ...
        "rebalance_orders": _execution_quality_summary(rebalance_rows),
        "protective_stops": _execution_quality_summary(protective_rows),
