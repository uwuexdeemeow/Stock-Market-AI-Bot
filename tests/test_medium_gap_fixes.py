...
def test_factor_decay_uses_non_overlapping_rebalance_cohorts():
    dates = pd.date_range("2026-01-02", periods=45, freq="B")
    scored = pd.DataFrame(
        {
            "date": [date for date in dates for _ticker in range(12)],
            "ticker": [f"T{ticker:02d}" for _date in dates for ticker in range(12)],
            "_monitor_score": [ticker for _date in dates for ticker in range(12)],
            "_monitor_return": [ticker / 100.0 for _date in dates for ticker in range(12)],
        }
    )
    ...
    assert cohorts["date"].nunique() == 3
    assert summary["cross_sections"] == 3
