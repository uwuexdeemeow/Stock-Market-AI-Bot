...
CONTRACT_SIGNAL = Path(__file__).parent / "fixtures" / "core_satellite_alpha_signal.csv"
...
class TestUnifiedSignalFile:
    ...
    def signal_path(self):
        # PLAIN ENGLISH: CI starts without ignored runtime signal files. Use a
        # committed contract example so these safety tests always run and never
        # depend on the age of a developer's local operational evidence.
    ...
    def test_predicted_at_is_timezone_aware(self, signal):
        """The contract timestamp must be parseable and identify its timezone."""
        ...
        assert ts.tzinfo is not None, "predicted_at must include an explicit timezone"
...
class TestSignalConsistency:
    ...
    def test_tqqq_weight_consistent_with_regime(self):
        ...
        sig = pd.read_csv(CONTRACT_SIGNAL).iloc[0]
    ...
    def test_unified_signal_exists(self):
        """The committed contract signal keeps clean CI coverage active."""
        assert CONTRACT_SIGNAL.exists()
        assert not pd.read_csv(CONTRACT_SIGNAL).empty, "Contract signal file is empty"
