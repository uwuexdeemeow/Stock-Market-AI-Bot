...
class TestSignalConsistency:
    ...
    def test_unified_signal_exists(self):
        """Validate the runtime signal when a pipeline run has generated it."""
        path = SIGNALS / "core_satellite_alpha_signal.csv"
        # A clean CI checkout intentionally has no live signal because signals
        # are runtime evidence, not source code. Other tests validate the
        # contract; this check inspects the real file only when one exists.
        if not path.exists():
            pytest.skip("Signal is generated by the paper pipeline")
        assert not pd.read_csv(path).empty, "Generated signal file is empty"
