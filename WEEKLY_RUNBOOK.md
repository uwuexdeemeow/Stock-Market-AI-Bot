...
| `--stable-grid` | off | Use the pinned consensus baseline grid (24 configs): pins h=20, overlay 50%, MA 100, regime-adaptive scoring, sticky-score weighting, and defensive risk control; only shape, high-vol mode, and TQQQ weight remain tunable. Auto-publishing is disabled unless `--publish-live-config` is passed. |
