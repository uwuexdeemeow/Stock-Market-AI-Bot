...
| `--stable-grid` | off | Use the consensus baseline grid (~48 configs): pins the 14-fold consensus dimensions (h=20, ma=100, regime_adaptive, risk=off) and searches only what actually varies across folds: overlay (50/70%), shape (top5/10/15), weighting (sticky_score/risk_parity), vol mode (fixed/percentile), and TQQQ (0/10%). Auto-publishing is disabled unless `--publish-live-config` is passed. |
...
For alpha-decay diagnosis, `--stable-grid` pins the 14-fold consensus
dimensions (`h=20`, `ma=100`, `score=regime_adaptive`, `risk=off`) and
searches only what varies across folds: overlay (50/70%), shape (top5/10/15),
weighting (sticky_score/risk_parity), vol mode, and TQQQ (0/10%).
This is an A/B research baseline and will not publish live approval state
unless forced with `--publish-live-config`.
